...
const EXAMPLES_TO_TRY: &[(&str, &str)] = &[
    ...
    (
        "where string",
        "SELECT count(*) FROM Transaction WHERE customerName = 'ReallyImportantCustomer' SINCE 1 day ago",
    ),
    (
        "latest duration",
    ...
    (
        "average duration",
        "SELECT average(duration) FROM Transaction SINCE 12 hours ago",
    ),
    (
        "percentile",
        "SELECT percentile(duration, 95) FROM Transaction SINCE 1 day ago",
    ),
    (
        "facet buckets numeric",
        "SELECT average(duration) FROM Transaction FACET buckets(duration, 400, 10) SINCE 12 hours ago",
    ),
    (
        "FROM first",
    ...
    (
        "limit offset",
        "FROM Log SELECT message LIMIT 100 OFFSET 50 SINCE 1 day ago",
    ),
    (
        "order by facet",
        "SELECT count(*) FROM Transaction FACET appName ORDER BY count(*) DESC LIMIT 5 SINCE 1 week ago",
    ),
    (
        "with timezone",
        "SELECT count(*) FROM Transaction WITH TIMEZONE 'America/New_York' SINCE 1 day ago",
    ),
    ("since now", "FROM Transaction SELECT * SINCE NOW"),
    (
        "backtick attr",
        "SELECT `user.id`, name FROM Transaction SINCE 1 day ago",
    ),
    (
        "where in",
        "SELECT * FROM Log WHERE level IN ('ERROR', 'WARN') SINCE 1 day ago",
    ),
    (
        "where numeric",
        "SELECT * FROM Transaction WHERE duration > 1000 SINCE 1 day ago",
    ),
    (
        "apdex two args",
        "SELECT apdex(duration, 0.4) FROM Transaction SINCE 1 day ago",
    ),
    (
        "multiple event types",
        "FROM Transaction, PageView SELECT count(*) SINCE 3 days ago",
    ),
    (
        "apdex named arg",
        "SELECT apdex(duration, t: 0.4) FROM Transaction WHERE customerName = 'x' SINCE 1 day ago",
    ),
    (
        "filter with WHERE",
        "SELECT filter(count(*), WHERE awsAPI = 'dynamodb') AS 'DynamoDB' FROM Public_APICall SINCE 1 day ago",
    ),
    (
        "rate with interval",
        "SELECT rate(count(*), 5 minutes) FROM Public_APICall SINCE 1 hour ago",
    ),
    (
        "funnel with WHERE",
        "SELECT funnel(awsAPI, WHERE http.url LIKE '%.amazonaws.com', WHERE http.url LIKE '%.us-west%') FROM Public_APICall SINCE 1 week ago",
    ),
    (
        "FACET CASES",
        "SELECT count(*) FROM Public_APICall FACET CASES(WHERE http.url LIKE '%amazon%', WHERE http.url LIKE '%google%') SINCE 1 day ago",
    ),
    (
        "EXTRAPOLATE",
...
const EXAMPLES_TO_TRY_2: &[(&str, &str)] = &[
    ...
    (
        "where web timeseries",
        "SELECT average(duration) FROM Transaction WHERE transactionType = 'Web' TIMESERIES AUTO",
    ),
    (
        "facet name",
    ...
    (
        ...
        "SELECT average(duration) FROM Transaction FACET name SINCE 3 hours ago LIMIT 5 TIMESERIES AUTO",
    ),
    (
        "facet limit 20",
    ...
    (
        "where facet limit timeseries",
        "SELECT count(*) FROM Transaction WHERE transactionType = 'Web' FACET appName LIMIT 5 SINCE 6 hours ago TIMESERIES AUTO",
    ),
    (
        "uniques",
    ...
    (
        "compare with timeseries",
        "SELECT average(duration) FROM Public_APICall SINCE 1 day ago COMPARE WITH 1 week ago TIMESERIES AUTO",
    ),
];
...
const EXAMPLES_TO_TRY_3: &[(&str, &str)] = &[
    (
        "median duration",
        "SELECT median(duration) FROM Transaction SINCE 1 day ago",
    ),
    (
        "stddev duration",
        "SELECT stddev(duration) FROM Transaction SINCE 24 hours ago",
    ),
    (
        "histogram three args",
        "SELECT histogram(duration, 1, 20) FROM Public_APICall SINCE 1 day ago",
    ),
    (
        "from log level message",
        "FROM Log SELECT level, message LIMIT 50 SINCE 2 hours ago",
    ),
    (
        "two event types NrdbQuery",
        "SELECT count(*) FROM NrdbQuery, NrDailyUsage SINCE 1 day ago",
    ),
    (
        "where response code",
        "SELECT average(duration) FROM Transaction WHERE httpResponseCode = 200 SINCE 1 day ago",
    ),
    (
        "where duration float",
        "SELECT count(*) FROM Transaction WHERE duration < 0.5 SINCE 1 hour ago",
    ),
    (
        "from metric sum",
        "FROM Metric SELECT sum(requests) SINCE 30 minutes ago",
    ),
    (
        "facet countryCode",
    ...
    (
        "three attrs limit",
        "SELECT name, duration, error FROM Transaction LIMIT 10 SINCE 1 day ago",
    ),
    (
        "facet two attrs",
        "SELECT count(*) FROM Transaction FACET appName, name LIMIT 5 SINCE 6 hours ago",
    ),
    (
        "where is null",
        "SELECT count(*) FROM Transaction WHERE error IS NULL SINCE 1 day ago",
    ),
    (
        "since until weeks",
        "SELECT average(duration) FROM Transaction SINCE 2 weeks ago UNTIL 1 week ago",
    ),
    ...
    (
        "compare with 6 hours",
        "SELECT average(duration) FROM Public_APICall SINCE 1 hour ago COMPARE WITH 6 hours ago",
    ),
    (
        "apdex with alias",
    ...
    (
        "where like pattern",
        "SELECT count(*) FROM Transaction WHERE name LIKE '%Controller%' SINCE 1 day ago",
    ),
    (
        "facet order asc",
        "SELECT count(*) FROM Transaction FACET appName ORDER BY appName ASC LIMIT 10 SINCE 1 day ago",
    ),
    (
        "timeseries 5 minutes",
        "SELECT average(duration) FROM Transaction SINCE 1 day ago TIMESERIES 5 minutes",
    ),
];
